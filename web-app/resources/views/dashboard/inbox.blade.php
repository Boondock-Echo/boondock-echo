...
@media screen and (max-width: 600px) {
  ...
  tr:nth-child(even) {
  background-color: rgba(237, 232, 232, 0.791);
}
}
...
{{-- for proccessed audio onclick colour change  --}}
<style>
    audio {
        width: 240px;
        height: 30px;
    ...
    /* Apply styles to the media controls panel of the processed audio element */
    audio.processed-audio::-webkit-media-controls-panel {
        background-color: rgb(238, 238, 238); /* Set the default color to grey */
    }

    /* Apply styles to the media controls panel of the processed audio element when checked */
    audio.processed-audio.blue-color::-webkit-media-controls-panel {
        background-color: #3a9dff; /* Change color to blue */
        color: white;
        
    }
</style>
{{-- style for mic recorder --}}
<style>
                ...
                <div class="col-sm-6 col-lg-6 mb-3 mb-lg-5">
                    ...
                    <div class="card h-100">
                        <div class="card-body">
                            ...
                            <div id="reloadButton" type="" class="h4" onclick="reloadPage()" style="display: none;">
                    ...
                    <div class="card-header card-header-content-md-between">
                        ...
                        <div class="d-grid d-sm-flex justify-content-md-end align-items-sm-center gap-2">
                            ...
                            <div class="form-check custom-switch form-switch mx-2">
                                <input type="checkbox" class="form-check-input " id="originalAudio" {{ auth()->user()->original_audio ? 'checked' : '' }}>
                                <label class="form-check-valid">Show Original</label>
                            </div>
                            
                            <script>
                                document.addEventListener('DOMContentLoaded', function () {
                                    const originalAudioCheckbox = document.getElementById('originalAudio');
                                    const originalAudioContainers = document.querySelectorAll('.original-audio-container');
                                    const processedAudios = document.querySelectorAll('.processed-audio');
                                    const dataTable = document.getElementById('datatable');
                                    const userId = {{ auth()->user()->id }};
                                    
                                    function updateOriginalAudioDisplay(isChecked) {
                                        originalAudioContainers.forEach(container => {
                                            container.style.display = isChecked ? 'inline' : 'none';
                                        });
                                        processedAudios.forEach(audio => {
                                            audio.classList.toggle('blue-color', isChecked);
                                        });
                                        dataTable.classList.toggle('show-original', isChecked);
                                    }
                            
                                    function updateOriginalAudioPreference(isChecked) {
                                        // Update local storage with user preference
                                        localStorage.setItem('originalAudioPreference', isChecked ? '1' : '0');
                                        
                                        $.ajax({
                                            url: '{{ route('user.updateOriginalAudio') }}',
                                            method: 'POST',
                                            data: {
                                                _token: '{{ csrf_token() }}',
                                                user_id: userId,
                                                original_audio: isChecked ? 1 : 0
                                            },
                                            success: function(response) {
                                                console.log('Original audio mode updated successfully.');
                                            },
                                            error: function(xhr) {
                                                console.log('An error occurred while updating original audio mode.');
                                            }
                                        });
                                    }
                            
                                    // Retrieve the user's original_audio preference from local storage
                                    const originalAudioPreference = localStorage.getItem('originalAudioPreference');
                                    if (originalAudioPreference === '1') {
                                        originalAudioCheckbox.checked = true;
                                        updateOriginalAudioDisplay(true);
                                    }
                            
                                    // Handle the change event of the original audio checkbox
                                    originalAudioCheckbox.addEventListener('change', function () {
                                        const isChecked = originalAudioCheckbox.checked;
                            
                                        updateOriginalAudioDisplay(isChecked);
                                        updateOriginalAudioPreference(isChecked);
                                    });
                                });
                            </script>
                            ...
                            <div class="form-check custom-switch form-switch mx-2">
                                <input type="checkbox" class="form-check-input is-valid" id="livemode" {{ auth()->user()->live_mode ? 'checked' : '' }}>
                                <label class="form-check-valid">Live Mode</label>
                            </div>
                              ...
                              <script>
                                $(document).ready(function() {
                                    ...
                                    $('#livemode').change(function() {
                                        ...
                                        // Send an AJAX request to update the live_mode value
                                        $.ajax({
                                            url: '{{ route('user.updateLiveMode') }}',
                                            method: 'POST',
                                            data: {
                                                _token: '{{ csrf_token() }}',
                                                user_id: userId,
                                                live_mode: isChecked ? 1 : 0
                                            },
                                            success: function(response) {
                                                console.log('Live mode updated successfully.');
                                            },
                                            error: function(xhr) {
                                                console.log('An error occurred while updating live mode.');
                                            }
                                        });
                                    });
                                });
                            ...
                            {{-- <div id="reloadButton" type="" class="" onclick="reloadPage()" style="display: none;">
...
<span data-bs-toggle="tooltip" data-bs-placement="top" title="Record Audio Here">
                            ...
                            {{-- recoredr mic end --}}
                            {{-- upload files start --}}
                            <span data-bs-toggle="tooltip" data-bs-placement="top" title="Upload">
                            <button type="button" class="btn btn-soft-primary " data-bs-toggle="modal"
                            ...
                            <tbody>
                                @foreach ($audioFiles as $key => $audioFile)
                                    <tr>
                                        ...
                                        {{-- </td> --}}
                                        <td data-label="Station" class="">
                                            <a class="text-dark" href="#">
                                                <!-- Add d-inline-flex class to create an inline flex container for the icon and title -->
                                                <div class="d-inline-flex align-items-center">
                                                    <span class="icon">
                                                        @if (in_array($audioFile->id, $sentMessageIds))
                                                            <i class="fa-solid fa-paper-plane text-success" data-bs-toggle="tooltip" data-bs-placement="top" title="Sent"></i>
                                                        @else
                                                            <i class="fa-solid fa-envelope text-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Received"></i>
                                                        @endif
                                                    </span>
                                                    <!-- Add ms-2 class to add a small margin on the right of the icon -->
                                                    <div class="ms-2">
                                                        <span data-bs-toggle="tooltip" title="Dock Name: {{ $dock->name }}">
                                                            ...
                                                            @foreach ($activeDocks as $dock)
                                                                ...
                                                                @if ($dock->mac == $audioFile->mac)  
                                                                    ...
                                                                    data-bs-toggle="modal" data-id="{{ $dock->id }}" data-bs-target="#sett{{ $dock->id }}"
                                        ...
                                        <td data-label="Audio" >
                                            ...
                                            <span>
                                                @if ($audioFile->org_audio)
                                                <span>
                                                    {{-- original --}}
                                                    <audio controls class="original-audio-container original-audio" style="width: 240px; height: 30px; display: none;" data-bs-toggle="tooltip" data-bs-placement="top" title="Original Audio">
                                                        <source src="{{config('app.cdn_server')}}/uploads/{{ $audioFile->mac }}/old/{{ $audioFile->file_name }}">
                                                    </audio>
                                                </span>
                                                @endif
                                                <audio controls style="width: 240px; height: 30px;" class="processed-audio my-2" data-bs-toggle="tooltip" data-bs-placement="top" title="Processed Audio">
                                                    <source src="{{config('app.cdn_server')}}/uploads/{{ $audioFile->mac }}/{{ $audioFile->file_name }}">
                                                </audio>
                                            </span>
                                        ...
                                        <td data-label="Actions" class="nowrap-td">
                                            ...
                                            {{-- @if (auth()->user()->hasRole('Super Admin')) --}}
                                                ...
                                                @else
                                                    <a class="btn btn-outline-danger btn-sm" data-bs-toggle="tooltip"
                                                        title='Favorite'
                                            ...
                                            @else
                                            @if (Auth::user()->call_sign)
                                            <a class="btn btn-outline-success btn-sm" data-bs-toggle="tooltip" title='Transmit'
                                                id="send-button{{ $audioFile->id }}">
                                                <i class="fa-solid fa-tower-broadcast"></i>
                                            </a>
                                        @else
                                            <a class="btn btn-outline-secondary btn-sm" data-bs-toggle="tooltip"
                                                title='Please update your ham radio license information on your profile'>
                                                <i class="fa-solid fa-tower-broadcast"></i>
                                            </a>
                                        @endif
                                            @endif
                    ...
                    class="btn btn-soft-danger">Yes, Delete</button>
                                            ...
                                            <a data-bs-toggle="tooltip" title="{{ $audioFile->transcribe_long }}">
                                                ...
                                                @if (empty($audioFile->transcribe_long))
                                                    <a href="{{ route('transcribe-audio', $audioFile->id) }}"
                                                        ...
                                                        class="btn btn-outline-info me-1 mb-1 btn-sm">
                                                ...
                                                @elseif(strlen($audioFile->transcribe_long) <= 30)
                                                    <span class="text-dark">{{ $audioFile->transcribe_long }}</span>
                                                @else
                                                    ...
                                                    <span id="transcription-full{{ $audioFile->id }}"
                                                        style="display:none" class="text-dark">
                                        ...
                                        <td data-label="Received">
                                            <a  @if (auth()->user()->hasRole('Admin'))
                                                @if (isset($audioFile) && !empty($audioFile))
                                                    data-bs-toggle="tooltip" data-bs-placement="top"  title="
                                                        @if(isset($audioFile->trigger_type)) Trigger: {{ $audioFile->trigger_type }}&#10; @else Trigger:-- &#10; @endif
                                                     @if(isset($audioFile->audio_end)) , Audio end: {{ $audioFile->audio_end }}&#10; @else Audio end:-- &#10; @endif
                                                        @if(isset($audioFile->duration)) , Duration: {{ $audioFile->duration }}&#10; @else Duration:-- &#10; @endif
                                                        @if(isset($audioFile->audio_level)),  Audio Level: {{ $audioFile->audio_level }}&#10; @else Audio Level:-- &#10; @endif
                                                        @if(isset($audioFile->file_size)), File size: {{ $audioFile->file_size }}&#10; @else File size:-- &#10; @endif
                                                    "
                                                @else
                                                    data-bs-toggle="tooltip" data-bs-placement="top" title="No data available"
                                                @endif
                                            @endif >
                                                @php
                ...
                <div id="sett{{ $dock->id }}" class="modal fade " tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5>
                                    {{ $dock->name ? $dock->name : 'No data found' }} ({{ $dock->mac ? $dock->mac : 'No data found' }}) 
                                      <span class="badge bg-primary rounded-pill ms-1">
                                        @php
                                            $licenseType = optional($dock->license_code)->license_type;
                                        @endphp
                                            ...
                                            @endphp
                                                <div class="tom-select-custom">
                                                    ...
                                                    @endphp
                                                        ...
                                                        @endforeach
                                                        @if (!$hasStations)
                                                        <option value="yes" selected>{{ $dock->station }} - {{ $dock->frequency }}{{ $dock->description }}</option>
                                                    @endif
