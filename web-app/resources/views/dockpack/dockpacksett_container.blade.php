<table class="js-datatable-checkboxes table  table-borderless table-thead-bordered table-nowrap table-align-center "
    ...
    <tbody id="data-table">
        ...
        @foreach ($dock_in_use as $dock)
            <form id="available-docks-form{{$id}}" method="POST" action="{{ route('update_available_docks') }}">
                @csrf
...
<div class="col-sm mb-2 mb-sm-0 mt-2">
    <div class="d-grid">
        <button class="btn btn-warning mx-2" type="button" id="settRemove-button{{ $id }}" data-dashlane-rid="d64f21caaacee0d1"> <i
...
<script>
    $(document).ready(function() {
        $('#settRemove-button{{ $id }}').click(function() {
            ...
            if (dock_ids.length > 0) {
                ...
                $.ajax({
                    url: '{{ route('update_available_docksett', ['id' => $id]) }}',
                    type: 'POST',
                    data: {
                        dock_ids: dock_ids,
                        in_use: 0,
