...
const distance = earthRadius * c/ 1.60934;
            ...
            // Function to display nearby fire stations within a 10000 miles radius from the user's location
            function allstationdisplay(userLocation) {
                ...
                nearbyallstations.forEach((station, index) => {
                    ...
                    // Check if the station is within a 10000 miles radius from the user's location
                    if (distance <= 90000) {
                        stationsFound = true; // Nearby stations found

                        // Check if the station location is already in the set (duplicate check)
                        if (!uniqueLocations.has(`${station.location}|${station.call}`)) {
                            const marker = new google.maps.Marker({
                                position: new google.maps.LatLng(station.lat, station.lng),
                                map: map,
                                title: `${station.location} (${station.distance} mile)`, // Combine location and distance
                                icon: {
                            ...
                            nameDiv.innerHTML = `
                                                            ...
                                                            <span class="text-dark fw-semibold">  ${station.location} - ${station.county}  <br>( ${station.distance} miles away ) </span>     <br>  Frequency : ${station.frequency} 
                                                        `;

                                                                        function getStationInfo(!uniqueLocations.has(`${station.location}|${station.call}`)){
                                                                            return `
